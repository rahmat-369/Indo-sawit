...
async function getNews() {
  // Dalam Next 15 Server Components, fetch ke absolute URL API internal bisa jadi isu saat build, 
  // jadi kita langsung panggil logic-nya atau pakai URL lengkap (disesuaikan dengan environment).
  // Buat aman di local & production Vercel:
  const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/news`, { cache: 'no-store' });
  if (!res.ok) return [];
  return res.json();
}
  ...
  return (
    <main className="min-h-screen bg-[#0B0F19] text-gray-100 selection:bg-green-500 selection:text-white pb-20">
      ...
      <nav className="sticky top-0 z-50 bg-[#0B0F19]/70 backdrop-blur-xl border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div>
          {/* Typografi Indosawit.Net dengan warna beda */}
        ...
        <div className="lg:col-span-3">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {newsList.length > 0 ? (
              ...
              <p className="text-gray-400 text-sm">Gagal memuat berita atau API sedang sibuk.</p>
            )}
          </div>
        </div>

        {/* Kanan: Developer Corner */}
        <aside>
          <DevCorner />
        </aside>
